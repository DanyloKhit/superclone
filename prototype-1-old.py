...
def apply_card(card):
    global player_max_hp, player_hp, player_fire_cooldown, player_damage, player_speed, active_cards, player_base_speed
    global player_crit_chance, player_lifesteal_amount, player_armor, special_unlocked
...
while running:
        ...
        if e.type == pygame.QUIT: running = False
            ...
            if state == "menu":
                if button_rect.collidepoint(e.pos):
                    ...
                    player_speed, player_base_speed, player_fire_cooldown = 5, 5, 250
                ...
                if player_shot_timer >= player_fire_cooldown:
                    ...
                    if 'Bulletstorm' in active_cards:
                        for angle_offset in [-0.15, 0, 0.15]:
                            ...
                            bullets.append({'x':cx, 'y':cy, 'vx':math.cos(angle)*12, 'vy':math.sin(angle)*12, 'r':4, 'owner':'player'})
                    else:
                        bullets.append({'x':cx, 'y':cy, 'vx':(dxp/dist)*12, 'vy':(dyp/dist)*12, 'r':4, 'owner':'player'})
    ...
    if state == 'menu':
        ...
        if not paused:
            dx, dy = (keys[pygame.K_d] - keys[pygame.K_a]) * player_speed, (keys[pygame.K_s] - keys[pygame.K_w]) * player_speed
            cx = max(cr, min(WIDTH-cr, cx+dx)); cy = max(cr, min(HEIGHT-cr, cy+dy))
            spawn_timer += scaled_dt; player_shot_timer += dt; player_dash_cooldown = max(0, player_dash_cooldown - dt)
            ...
            for en in enemies[:]:
                en['shot_timer'] += scaled_dt; en['strafe_timer'] += scaled_dt
                if en['type'] == 'teleporter':
                    en['teleport_timer'] += scaled_dt
                ...
                vx_des = (dxp/dist) * spd - 0.5*sepx
                vy_des = (dyp/dist) * spd - 0.5*sepy
                if dist < 160:
                ...
                en['x'] += vx_des * time_scale; en['y'] += vy_des * time_scale
